...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, use_lora=False, use_adapter=False, 
        ...
        self.rho = 0.99
        ## PEFT modules
        self.pissa_B1 = nn.Parameter(torch.empty(4*dim, self.rank))
        self.pissa_A1 = nn.Parameter(torch.empty(self.rank, dim))
        self.pissa_B2 = nn.Parameter(torch.empty(dim, self.rank))
        self.pissa_A2 = nn.Parameter(torch.empty(self.rank, 4*dim))
        self._pissa_init()
        ## init A and B
    
    def _pissa_init(self):
        print("use pissa initialize ...")
        ori_weight_1 = self.pwconv1.weight.clone()  # [4c, c]
        ori_weight_2 = self.pwconv2.weight.clone()  # [c, 4c]
        u1, s1, v1 = torch.svd(ori_weight_1)
        u2, s2, v2 = torch.svd(ori_weight_2)

        u1_r, s1_r, v1_r = u1[:, :self.rank], s1[:self.rank]/self.scaling, v1[:, :self.rank]
        u2_r, s2_r, v2_r = u2[:, :self.rank], s2[:self.rank]/self.scaling, v2[:, :self.rank]
        # LoRA A1 B1
        lora_b1 = u1_r * torch.sqrt(s1_r).unsqueeze(0) 
        lora_a1 = torch.sqrt(s1_r).unsqueeze(1) * v1_r.transpose(0, 1)
        # LoRA A2 B2
        lora_b2 = u2_r * torch.sqrt(s2_r).unsqueeze(0)
        lora_a2 = torch.sqrt(s2_r).unsqueeze(1) * v2_r.transpose(0, 1)

        self.pissa_B1.data.copy_(lora_b1)
        self.pissa_A1.data.copy_(lora_a1)
        self.pissa_B2.data.copy_(lora_b2)
        self.pissa_A2.data.copy_(lora_a2)

        self.pwconv1.weight.data.copy_(ori_weight_1 - self.scaling*lora_b1 @ lora_a1)
        self.pwconv2.weight.data.copy_(ori_weight_2 - self.scaling*lora_b2 @ lora_a2)
        self.pwconv1.weight.requires_grad_(False)
        self.pwconv2.weight.requires_grad_(False)
        
    def compute_delta_w(self):
        delta_w1 = (self.pissa_B1 @ self.pissa_A1) * self.pwconv1.weight.detach() * self.scaling
        delta_w2 = (self.pissa_B2 @ self.pissa_A2) * self.pwconv2.weight.detach() * self.scaling
