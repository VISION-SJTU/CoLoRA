...
class LoRA(nn.Module):
    def __init__(self, in_dim, out_dim, reduction=16,stride=1,kernel_size=1, use_filter=False, wsize=11, sigma=1, scale_factor=0.1, use_depth=False, use_pool=False, alpha=16): # wsize larger better
        ...
        if use_depth:
            self.lora_B = nn.Conv2d(self.rank, out_dim, 1, stride=stride, padding=kernel_size//2, bias=False, groups=self.rank)
        else:
...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, use_lora=False, use_adapter=False, 
        ...
        ## PEFT modules
        if self.use_lora:
            self.lora = LoRA(dim, dim, reduction=self.lora_reduction, use_filter=True, wsize=11, sigma=1.0, scale_factor=0.2, use_depth=True, alpha=self.alpha)
            self.lora_pw_B1 = nn.Parameter(torch.empty(4*dim, self.rank//2))
            self.lora_pw_A1 = nn.Parameter(torch.empty(self.rank//2, dim))
            self.lora_pw_B2 = nn.Parameter(torch.empty(dim, self.rank//2))
            self.lora_pw_A2 = nn.Parameter(torch.empty(self.rank//2, 4*dim))
            self.lora_pw_share_B = nn.Parameter(torch.empty(dim, self.rank//2))
            self.lora_pw_share_A = nn.Parameter(torch.empty(self.rank//2, dim))
        ...
        if self.use_lora:
            if not merge_weights:
                ...
                with torch.cuda.amp.autocast(enabled=False):
                    ...
                    # nn.init.kaiming_uniform_(self.lora_pw_B2)
                ...
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=False):
                        ...
                        # delta_w1, delta_w2 = self.compute_delta_w()
                        # self.pwconv1.weight.data.copy_(self.ori_weight_1)
                        # self.pwconv2.weight.data.copy_(self.ori_weight_2)
                        # self.ori_weight_1 = self.pwconv1.weight.data
                        # self.ori_weight_2 = self.pwconv2.weight.data
                        # reinitialize
                        nn.init.zeros_(self.lora_pw_B1)
                        nn.init.zeros_(self.lora_pw_B2)
                        nn.init.zeros_(self.lora_pw_share_B)
    ...
    def compute_delta_w(self, scaling=None):
         with torch.cuda.amp.autocast(enabled=False):
            ...
            scale_share = (self.rank/2 * self.dim) ** 0.25 * (self.lora_pw_share_A.std() + self.lora_pw_share_B.std())
            
            scale_ab1 = (self.rank/2 * self.dim) ** 0.25 * (self.lora_pw_A1.std() + self.lora_pw_B1.std())
            scale_ab2 = (self.rank/2 * self.dim) ** 0.25 * (self.lora_pw_A2.std() + self.lora_pw_B2.std())
            ## scaling strategy
            ...
            indep_delta1 = self.lora_pw_B1 @ self.lora_pw_A1 * 1 / scale_ab1.detach()
            indep_delta2 = self.lora_pw_B2 @ self.lora_pw_A2 * 1 / scale_ab2.detach()
