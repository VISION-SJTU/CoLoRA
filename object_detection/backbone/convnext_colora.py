...
# --- Fused trig function with in-place ops & fewer temporaries ---
import torch
import torch.nn as nn
import torch.nn.functional as F
import math
...
@torch.jit.script
def fast_sin_cos(x):
    return torch.sin(x).clamp_min(0.0), torch.cos(x).clamp_min(0.0)
...
class BilateralModule(nn.Module):
    def __init__(self, wsize=11, sigma=0.1, scale_factor=0.1):
        ...
        # Register Gaussian kernel as buffer (no gradient, moved with .cuda/.to)
        kernel = self._make_gaussian_kernel(wsize, sigma)
        self.register_buffer("kernel", kernel)  # [1, 1, k, k]

    def _make_gaussian_kernel(self, ksize, sigma):
        center = (ksize - 1) / 2
        grid = torch.arange(ksize, dtype=torch.float32)
        grid = grid - center
    ...
    def forward(self, x):
        with torch.cuda.amp.autocast():
            ...
            # Compute gamma scaling
            xmax = x.amax(dim=[-2, -1], keepdim=True)
            xmin = x.amin(dim=[-2, -1], keepdim=True)
            T = (xmax - xmin).clamp(min=1e-5)  # avoid div by 0
            gamma = math.pi / (2 * T)

            # Apply trig transform once and reuse
            gx = gamma * x
            # sinx = torch.sin(gx).clamp_(min=0)
            # cosx = torch.cos(gx).clamp_(min=0)
            sinx, cosx = fast_sin_cos(gx)
            sinx, cosx = sinx.detach(), cosx.detach()

            # Pad only once
            pad = self.wsize // 2
            x_pad = F.pad(x, (pad, pad, pad, pad), mode='reflect')
            sinx_pad = F.pad(sinx, (pad, pad, pad, pad), mode='reflect')
            cosx_pad = F.pad(cosx, (pad, pad, pad, pad), mode='reflect')

            # Expand kernel only once
            kernel = self.kernel.expand(C, 1, -1, -1)  # [C, 1, k, k]

            # Convolutions (grouped per channel)
            x_cos = cosx * F.conv2d(cosx_pad * x_pad, kernel, groups=C)
            x_sin = sinx * F.conv2d(sinx_pad * x_pad, kernel, groups=C)
            norm_cos = cosx * F.conv2d(cosx_pad, kernel, groups=C)
            norm_sin = sinx * F.conv2d(sinx_pad, kernel, groups=C)

            output = (x_cos + x_sin) / (norm_cos + norm_sin + 1e-5)
        return shortcut + self.scale_factor * output
...
class LoRA(nn.Module):
    def __init__(self, in_dim, out_dim, reduction=16,stride=1,kernel_size=1, use_filter=False, wsize=11, sigma=1, scale_factor=0.1, use_depth=False, use_pool=False, alpha=16): # wsize larger better
        ...
        self._init()
        if self.use_filter:
            self.filtering = torch.jit.script(BilateralModule(wsize, sigma=sigma, scale_factor=scale_factor))
...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, use_lora=False, use_adapter=False, 
        ...
        ## PEFT modules
        if self.use_lora:
            self.lora = LoRA(dim, dim, reduction=self.lora_reduction, use_filter=use_filter, wsize=11, sigma=1.0, scale_factor=0.2, use_depth=True, alpha=self.alpha)
        if self.use_lora:
    ...
    def compute_delta_w(self, scaling=None):
        device = self.lora_pw_share_A.device
        dim_sqrt = torch.sqrt(torch.tensor(self.dim, dtype=torch.float32, device=device))
        
        # Shared branch
        if self.share_rank > 0:
            share_scale_factor = (self.share_rank * self.dim)**0.25

            W1 = self.ori_weight_1.detach().to(device)
            W2 = self.ori_weight_2.detach().to(device)

            tmp_B = torch.matmul(W2.T, self.lora_pw_share_B)  # shape: [dim, r]
            tmp_A = torch.matmul(self.lora_pw_share_A, W1.T)  # shape: [r, dim]

            std_A = self.lora_pw_share_A.std()
            std_B = self.lora_pw_share_B.std()

            scale_share_1 = share_scale_factor * (tmp_B.std() + std_A)
            scale_share_2 = share_scale_factor * (tmp_A.std() + std_B)

            share_delta1 = torch.matmul(tmp_B, self.lora_pw_share_A / scale_share_1)
            share_delta2 = torch.matmul(self.lora_pw_share_B / scale_share_2, tmp_A)
        else:
            share_delta1 = share_delta2 = 0

        # Independent branch
        if self.indep_rank > 0:
            indep_scale_factor = torch.sqrt(torch.tensor(self.indep_rank, dtype=torch.float32, device=device))

            std_A1 = self.lora_pw_A1.std()
            std_B1 = self.lora_pw_B1.std()
            std_A2 = self.lora_pw_A2.std()
            std_B2 = self.lora_pw_B2.std()

            scale_ab1 = indep_scale_factor * (std_A1 + std_B1)
            scale_ab2 = indep_scale_factor * (std_A2 + std_B2)

            indep_delta1 = torch.matmul(self.lora_pw_B1, self.lora_pw_A1) / scale_ab1
            indep_delta2 = torch.matmul(self.lora_pw_B2, self.lora_pw_A2) / scale_ab2
        else:
            indep_delta1 = indep_delta2 = 0
